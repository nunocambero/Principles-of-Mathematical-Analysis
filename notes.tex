...
Consider a semi-algebra \(\mathcal{E}\) on \(X\) and a countably additive function \(\mu_0: \mathcal{E} \to [0, \infty]\). We define for all \(A \in \mathcal{P}(X)\):
\[\mu^*(A) = \inf\left\{\sum_{j=1}^{\infty} \mu_0(E_j) : A \subseteq \bigcup_{j=1}^{\infty} E_j, \quad E_j \in \mathcal{E}\right\}.\]
