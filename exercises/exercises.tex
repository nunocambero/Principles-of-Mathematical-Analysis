...
\textbf{\large Consider the function
\[f(x) = \frac{\sin x}{x}, \quad x \in (0, \infty).\]
Prove that \(f \notin L^1((0, \infty))\)}

\textbf{Hint:} divide the interval \((0, \infty)\) into subintervals of the form \([n\pi, (n+1)\pi]\), for \(n = 1, 2, \ldots\)
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        samples=200,
        ytick={0, 1},
        xtick={0, 0.5*pi, pi, 2*pi, 3*pi, 4*pi},
        grid=both,
        legend pos=north east
    ]
        \addplot[thick, color=blue] {sin(deg(x))/x};
        \addplot[thick, dashed, color=red] {1/x};
...
We divide the interval \((0, \infty)\) into subintervals of the form \([n\pi, (n+1)\pi]\), for \(n = 1, 2, \ldots\):
\begin{center}
    \begin{tikzpicture}
        % Draw the real line
        \draw[thick, ->] (-1, 0) -- (13, 0) node[right] {};
        ...
        % Label subintervals
        \node at (4.71, 0.5) {$(\pi, 2\pi]$};
        \node at (10.71, 0.5) {$(n\pi, (n + 1)\pi]$};
    \end{tikzpicture}
\end{center}

We have:
\[\int_{0}^{\infty} \left|\frac{\sin x}{x}\right| \, dx = \sum_{n=0}^{\infty} \int_{n\pi}^{(n+1)\pi} \left|\frac{\sin x}{x}\right| \, dx \geq \sum_{n=0}^{\infty} \frac{1}{(n + 1)\pi} \int_{n\pi}^{(n+1)\pi} |\sin x| \, dx = \]
\[= \int_{0}^{\pi} \sin x \, dx \cdot \frac{1}{\pi} \sum_{n=0}^{\infty} \frac{1}{(n + 1)} = \left[\cos x\right]_{0}^{\pi} \cdot \frac{1}{\pi} \sum_{n=1}^{\infty} \frac{1}{n} = \dfrac{2}{\pi} \cdot \infty = \infty.\]

Thus, \(f \notin L^1((0, \infty))\).

\section*{Exercise 2.2.6}
\textbf{\large Consider the function
\[f(x) = \frac{1 - \cos x}{x(1 + x^2)}, \quad x \in (0, \infty).\]
Prove that \(f \in L^1((0, \infty))\).}

We have:
\[\int_{0}^{\infty} \left|\frac{1 - \cos x}{x(1 + x^2)}\right| \, dx = \int_{0}^{\infty} \frac{1 - \cos x}{x(1 + x^2)} \, dx\]

Using the Taylor expansion of \(\cos x\) around \(0\):
\[\cos x = 1 - \frac{x^2}{2} + \dots \implies 1 - \cos x = \frac{x^2}{2} + \varepsilon \quad \text{on some } (0, \delta),\]

Then in \((0, \delta)\):
\[\left| \frac{1 - \cos x}{x^2 / 2} \right| < 1 + \varepsilon\]
And,
\[|f(x)| < \frac{(1 + \varepsilon) x^2 / 2}{x(1 + x^2)} = \frac{(1 + \varepsilon) x}{2(1 + x^2)} \in L^1((0, \delta)).\]

On \((\delta, \infty)\), we have:
\[|f(x)| \leq \frac{2}{x(1 + x^2)} \leq \frac{2}{x^3} \in L^1((\delta, \infty)).\]
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        \addplot[thick, color=blue, domain=0:4, samples=200] {(1 - cos(deg(x))) / (x * (1 + x^2))};
        \addplot[thick, dashed, color=red, domain=0.3:4, samples=200] {2 / (x^3)};
        \addplot[thick, dashed, color=orange, domain=0.01:4, samples=200] {(1.1 * x) / (2 * (1 + x^2))};

    \addlegendentry{\(f(x) = \frac{1 - \cos x}{x(1 + x^2)}\)}
    \addlegendentry{\(g(x) = \frac{2}{x^3}\)}
    \addlegendentry{\(h(x) = \frac{(1 + \varepsilon)x}{2(1 + x^2)}\)}
    \end{axis}
\end{tikzpicture}
\end{center}
